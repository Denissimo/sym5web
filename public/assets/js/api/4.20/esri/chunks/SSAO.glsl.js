...
"sampler2D").add("depthMap","sampler2D").add("tex","sampler2D").add("blurSize","vec2").add("g_BlurFalloff","float").add("projScale","float").add("nearFar","vec2").add("zScale","vec2"),a.fragment.code.add(b.glsl`
      ...
      void main(void) {
        ...
        for (int r = -${b.glsl.int(c.radius)}; r <= ${b.glsl.int(c.radius)}; ++r) {
          float rf = float(r);
    ...
    uniform vec3 pSphere[${b.glsl.int(c.samples)}];
